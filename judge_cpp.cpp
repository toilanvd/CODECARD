...
string forbid_cmd_funcname[] = {"close", "open", "freopen", "fopen", "fclose", "ifstream", "ofstream",
                        "fork", "exec", "pipe", "waitpid", "vfork",
                        "socket", "setsockopt", "bind", "listen", "accept", "connect",
                        "wait", "sleep", "clock", "signal", "sig", "pause", "poll", "select",
                        "alarm", "gettime", "sync", "shmget", "msgget", "mq_open", "kill", "system", "sys", "-1"};

string forbid_cmd[] = {"close@", "open@", "freopen@", "fopen@", "fclose@", "ifstream", "ofstream",
                        "fork@", "exec@", "pipe@", "waitpid@", "vfork@",
                        "socket@", "setsockopt@", "bind@", "listen@", "accept@", "connect@",
                        "wait@", "sleep@", "clock@", "signal@", "sig@", "pause@", "poll@", "select@",
                        "alarm@", "gettime@", "sync@", "shmget@", "msgget@", "mq_open@", "kill@", "system@", "sys@", "-1"};

string forbid_word[] = {"cerr<","cerr <","cerr  <","cerr   <","-1"};

string forbid_func[] = {"cerr", "cerr", "cerr", "cerr", "-1"};
...
//        compile_option[cnt] = ' '; cnt++;
    ...
    sprintf(buf, "code/%s.cpp -o logs/%s 2> logs/%s.error.txt", filename, filename, filename);
...
//            cnt++;
    ...
    // Compile
//    sprintf(buf, "g++ %s code/%s.cpp -o logs/%s 2> logs/%s.error.txt", compile_option, filename, filename, filename);
//    system(buf);
    ...
    while(fgets(content, 500, compile_result)!=NULL){
        for(int i=0;i<=(int)strlen(content)-5;i++)
            if(content[i]=='e' && content[i+1]=='r' && content[i+2]=='r' && content[i+3]=='o' && content[i+4]=='r')
                compile_error = true;
    ...
    sprintf(buf, "objdump -d logs/%s > logs/%s.assembly.txt", filename, filename);
    system(buf);
    ...
    while(fgets(content, 500, assembly_file)!=NULL){
        for(int i=0;i<(int)strlen(content);i++){
            for(int j=0;;j++){
                ...
                if(!different){
                    code_violation = true;
                    fprintf(judge, "violation: Please remove '%s' function\n", forbid_cmd_funcname[j].c_str());
                    printf("violation: Please remove '%s' function\n", forbid_cmd_funcname[j].c_str());
                    break;
                }
            }
    ...
    sprintf(buf, "code/%s.cpp", filename);
    assembly_file = fopen(buf, "r");
    int line_of_code = 0;
    while(fgets(content, 500, assembly_file)!=NULL){
        line_of_code++;
        for(int i=0;i<(int)strlen(content);i++){
            for(int j=0;;j++){
                if(strcmp(forbid_word[j].c_str(),"-1")==0) break;
                if(i+forbid_word[j].length()-1 >= strlen(content)) continue;
                bool different = false;
                for(int k=i;k<=i+forbid_word[j].length()-1;k++){
                    if(content[k]!=forbid_word[j][k-i]){
                        different = true;
                        break;
                    }
                }
                if(!different){
                    code_violation = true;
                    fprintf(judge, "Line %d: violation: Please remove '%s'\n", line_of_code, forbid_func[j].c_str());
                    printf("Line %d: violation: Please remove '%s'\n", line_of_code, forbid_func[j].c_str());
                    break;
                }
            }
            if(code_violation) break;
